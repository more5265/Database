...
#요구사항1 모든 의사의 의사ID, 이름, 생년월일, 전공의과 번호, 전공의과 이름을 조회
SELECT 
	`doc_id`,
	`doc_name`,
	`doc_birth`,
...
#요구사항2 모든 간호사의 간호사ID, 이름, 생년월일, 전공의과 번호, 전공의과 이름을 조회
SELECT
	`nur_id`,
	`nur_name`,
	`nur_birth`,
...
#요구사항3 모든 환자의 환자ID, 이름, 주민번호, 휴대폰, 담당의사 이름, 담당 간호사 이름 조회
SELECT
	`pat_id`,
	`pat_name`,
	`pat_jumin`,
...
#요구사항4 모든 진료내역에서 환자 이름, 담당의사 이름, 진료내용, 처방내용, 진료날짜를 조회
SELECT
	`pat_name`,
	`doc_name`,
	`treat_contents`,
...
#요구사항5 모든 진료내역에서 외과에서 진료한 내역 가운데 진료번호, 환자이름, 담당의사명, 진료내용, 처방내용 진료날짜를 조회
SELECT
	a.treat_no,
	`pat_name`,
	`doc_name`,
...
#요구사항6 모든 진료내역에서 화상으로 진료한 내역 가운데 진료번호, 환자이름, 담당의사명, 진료내용, 처방내용 진료날짜를 조회
SELECT 
	a.treat_no,
	`pat_name`,
	`doc_name`,
...
#요구사항7 현재 날짜를 기준으로 30세 이상 ~ 40세 미만의 환자들을 조회

SELECT *
FROM `patients`
WHERE FLOOR(DATEDIFF(CURRENT_DATE(), STR_TO_DATE(SUBSTRING(`pat_jumin`, 1, 6), '%y%m%d')) / 365.25) >= 30
  AND FLOOR(DATEDIFF(CURRENT_DATE(), STR_TO_DATE(SUBSTRING(`pat_jumin`, 1, 6), '%y%m%d')) / 365.25) < 40;
  
SELECT *
FROM `patients`
WHERE `pat_jumin` BETWEEN DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 40 YEAR), '%y%m%d')
                     AND DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 30 YEAR), '%y%m%d');
...
#요구사항8 모든 전공부서에서 아직 의사ID가 부여되지 않은 의사의 이름과 부서명을 조회
SELECT 
	`dep_manager`,
	`dep_name`
...
#요구사항9 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자 ^로 해서 조회 (ex '강동원^고소영^이나영)
SELECT 
	GROUP_CONCAT(`pat_name` SEPARATOR '^') AS `담당 환자들`
FROM `patients` AS a
JOIN `Nurses` AS b ON a.nur_id = b.nur_id
WHERE `nur_name` = '김태희';


#요구사항10 가장 많은 환자 수를 담당하는 간호사ID, 간호사이름, 담당환자수를 조회
SELECT 
	a.nur_id,
	`nur_name`,
	COUNT(*) AS `환자 수`
